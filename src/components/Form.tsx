    ...
    return (
        ...
        className="space-y-5 text-slate-800 grow text-right">
            <div className="relative">
                ...
                <label className={`input ${username ? "translate-y-[-125%] scale-[80%]" : ""}`}>Your name</label>
            </div>
            
            <div className="relative">
                <input value={email} onChange={(e) => setEmail(e.target.value)}
                className="w-full h-10 px-4 py-2 bg-transparent border-2 outline-none peer border-slate-600 hover:border-slate-600 focus:border-slate-50 text-slate-50"/>
                <label className={`input ${email ? "translate-y-[-125%] scale-[80%]" : ""}`}>Your email</label>
            </div>

            <div className="relative">
                <textarea name="" id="" cols={10} value={message} onChange={(e) => setMessage(e.target.value)}
                className="w-full px-4 py-2 bg-transparent border-2 outline-none min-h-1/4 peer border-slate-600 hover:border-slate-600 focus:border-slate-50 text-slate-50"></textarea>
                <label className={`input ${message ? "translate-y-[-125%] scale-[80%]" : ""}`}>Your message</label>
            </div>
